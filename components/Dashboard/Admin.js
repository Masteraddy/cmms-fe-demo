...
import { siteData, emailData } from '../../lib/data-helper';
...
const Overview = (props) => {
  ...
  const generateFilter = () => {
    let arr = [];
    let Completed = props.requests.requests.filter(
      (dt) => dt.status === 'done',
    );
    axes.map((x) => {
  ...
  var requests = props.requests.requests.filter(
    (dt) => dt.assignedId !== 'Unassigned',
  );
