...
const Chat = (props) => {
  ...
  const myContacts = () => {
    ...
    props.authentication.users.map((cont) => {
      if (
        (cont.usertype === 'manager' ||
          cont.usertype === 'team-member' ||
          cont.usertype === 'procurement' ||
          cont.usertype === 'technician') &&
        ...
        arr.push({
          ...
          avatar: (
            <Avatar
              size={48}
              src={
                cont.picture !== '' ? cont.picture : '/static/images/avatar.jpg'
              }
            />
          ),
        });
          ...
          {!state.mobile && (
            ...
            >
              <span
                ...
                `}
              >
                {props.authentication.user.firstname}{' '}
                {props.authentication.user.lastname}
              </span>
            ...
            <ChatStyled>
              <React.Fragment>
                {currentChat === '' ? (
                  ...
                  props.chats
                    .filter((dt) => dt.chatId == currentChat)
                    .map((chat, index) => (
                      <div
                        key={index}
