...
import { editProfile, editUserPass } from '../redux/actions';
...
const Social = ({
  ...
  const editPassword = editUserPass;
  ...
  const fetchIcon = (status) => {
    ...
    switch (val) {
      case 'pending':
        icon = (
          <AlertCircle size={16} strokeWidth={1} color={theme.errorColor} />
        );
        break;
      case 'active':
                ...
                <Col>
                  <div className='text-center'>
                    <Avatar
                      size={80}
                      src={
                        picture !== '' ? picture : '/static/images/avatar.jpg'
                      }
                    />
                  </div>
          ...
          {activeTab === '2' && (
            <Card title='Tasks'>
              <Timeline pending='Tasks pending...' className='mt-2'>
                {requests.requests.slice(0, 10).map((request) => (
                  <Timeline.Item
                    key={request._id}
          ...
          {activeTab === '4' && (
            <Card>
              ...
              <Uploader
                uploadResponse={(data) => {
                  if (data.picture)
                    return editProf({ picture: data.picture }, _id);
                }}
              />
              <hr />
              {!changePassword && (
...
export default connect((state) => state, { editProfile, editUserPass })(
  Form.create()(Social),
);
